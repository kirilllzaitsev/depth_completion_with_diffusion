...
class ConvNextBlock(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, *, time_emb_dim=None, mult=2, norm=True):
        super().__init__()
        self.mlp = (
...
class Unet(nn.Module):
    def __init__(
        ...
        time_dim_hidden,
        first_hidden_dim,
        final_conv_hidden_dim,
        out_dim,
        hidden_dim_scalers=(1, 2, 4, 8),
        channels=3,
        with_time_emb=True,
        ...
        self.init_dim = first_hidden_dim
        self.init_conv = nn.Conv2d(channels, self.init_dim, 7, padding=3)

        self.hidden_dims = [
            self.init_dim,
            *map(lambda m: self.init_dim * m, hidden_dim_scalers),
        ]
        in_out = list(zip(self.hidden_dims[:-1], self.hidden_dims[1:]))
        ...
        # time embeddings
        self.time_dim_input = time_dim_input
        self.time_dim_hidden = time_dim_hidden
        self.time_mlp = nn.Sequential(
            SinusoidalPositionEmbeddings(self.time_dim_input),
            nn.Linear(self.time_dim_input, self.time_dim_hidden),
            nn.GELU(),
            nn.Linear(self.time_dim_hidden, self.time_dim_hidden),
        )
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        block_klass(dim_in, dim_out, time_emb_dim=self.time_dim),
                        block_klass(dim_out, dim_out, time_emb_dim=self.time_dim),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        block_klass(dim_out * 2, dim_in, time_emb_dim=self.time_dim),
        ...
        self.final_conv = nn.Sequential(
            block_klass(dim_in=self.init_dim, dim_out=final_conv_hidden_dim),
            nn.Conv2d(final_conv_hidden_dim, self.out_dim, 1),
        )
