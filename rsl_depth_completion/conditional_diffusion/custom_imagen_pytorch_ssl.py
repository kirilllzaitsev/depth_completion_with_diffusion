...
class Imagen(nn.Module):
    def __init__(
        ...
        *,
        ...
        ), "downsample factor of last stage must be 1"
        assert (
            tuple(sorted(temporal_downsample_factor, reverse=True))
        ...
        self.min_snr_gamma = tuple(
            (gamma if use_min_snr else None)
            for use_min_snr, gamma in zip(min_snr_loss_weight, min_snr_gamma)
        )
    ...
    def sample(
        ...
        frames_to_resize_kwargs = (
            lambda frames: dict(target_frames=frames) if exists(frames) else dict()
        )
        ...
        if start_at_unet_number > 1:
            ...
            prev_frame_size = (
                all_frame_dims[start_at_unet_number - 2][0] if self.is_video else None
            )
            img = self.resize_to(
                start_image_or_video,
                prev_image_size,
        ...
        for (
            ...
            context = (
                ...
                if is_cuda and use_one_unet_in_gpu
                else nullcontext()
            )
            ...
            with context:
                ...
                if self.is_video and self.resize_cond_video_frames:
                    ...
                    video_kwargs = maybe_transform_dict_key(
                        video_kwargs, "cond_video_frames", temporal_downsample_fn
                    )
                    video_kwargs = maybe_transform_dict_key(
                ...
                resize_kwargs = (
                    dict(target_frames=frame_dims[0]) if self.is_video else dict()
                )
    ...
    def p_losses(
        ...
        **kwargs,
        ...
        # normalize to [-1, 1]
        input_x = self.normalize_img(x_start)
        ...
        if pred_objective == "noise":
            ...
            print(f"{pred_objective=}. ensure x_denoised is correct.")
    ...
    def forward(
        ...
        **kwargs,
        ...
        all_frame_dims = tuple(
            ...
            for el in calc_all_frame_dims(self.temporal_downsample_factor, frames)
        )
        ignore_time = kwargs.get("ignore_time", False)

        target_frame_size = (
            all_frame_dims[unet_index] if is_video and not ignore_time else None
        )
        prev_frame_size = (
            all_frame_dims[unet_index - 1]
            if is_video and not ignore_time and unet_index > 0
            else None
        )
        frames_to_resize_kwargs = (
            lambda frames: dict(target_frames=frames) if exists(frames) else dict()
        ...
        if self.is_video and self.resize_cond_video_frames:
            ...
            kwargs = maybe_transform_dict_key(
                kwargs, "post_cond_video_frames", temporal_downsample_fn
            )
        ...
        images = self.resize_to(
            images, target_image_size, **frames_to_resize_kwargs(target_frame_size)
        )
