...
def plot_losses(losses, rec_losses):
    ...
    rename_map = {"triplet_loss": "ssl_loss", "imagen_loss": "diffusion_loss"}
    for i, (k, v) in enumerate(losses.items()):
        axs[i].plot(v)
        title = rename_map.get(k, k)
    ...
    plt.show()
    pd.DataFrame(rec_losses).rename(columns={
        "image_01_loss": "image_t-1_loss",
        "image_02_loss": "image_t+1_loss",
    }).plot(
        title="Reconstruction losses", figsize=(4, 3), xlabel="Step", ylabel="Loss"
    )
...
def plot_grads(depth_grads, pose_grads, topk=5, bottomk=5):
    ...
    def display_grads(grads, ax, title):
        ...
        try:
            pd.DataFrame({k: v for k, v in grads.items() if v}).plot(
                title=title, ax=ax, legend=False
            )
        except:
...
def plot_train_depths_overall(
    output_depths, save_dir=None, save_subdir_prefix="", ckpts=None
):
    figs = []
    ...
    for idx in range(len(batch_size)):
        ...
        n_rows = (
            len(pred_ckpts) // 5
            if len(pred_ckpts) % 5 == 0
            else len(pred_ckpts) // 5 + 1
        )
        n_cols = min(5, len(pred_ckpts))
        fig, axs = plt.subplots(n_rows, n_cols, figsize=(20, 10))
        for i, ckpt in enumerate(pred_ckpts):
            pred = output_depths[i][idx].detach().cpu().numpy().squeeze()
            if n_rows == 1:
                if n_cols == 1:
                    ax = axs
                else:
