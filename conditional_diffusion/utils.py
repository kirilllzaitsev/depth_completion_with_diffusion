...
def print_metrics(mae, rmse, imae, irmse, comment="", save_csv=False, save_csv_dir=None):
    mae_mean, mae_std = calc_mean_std(mae)
    rmse_mean, rmse_std = calc_mean_std(rmse)
    imae_mean, imae_std = calc_mean_std(imae)
    irmse_mean, irmse_std = calc_mean_std(irmse)

    # Print evaluation results to console and file
    print(f"Evaluation results ({comment})")
    print(f"Lowest MAE: {np.min(mae)}")
